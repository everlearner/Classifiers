...
def sgd(x, y_org, lr, num_iter ):
    '''
    Function to apply Stochastic Gradient descent on training data, and find weights 
    '''
    w = np.ones((4,1))
    xaxis = []
    error_to_plot = []
    ...
    for i in range(0, num_iter):
        ...
        diff = pdt[index] - y_org[index]
        grad = (diff * x[index]).reshape(w.shape)
        #print(grad)
        w = w - lr*(grad)
        pdt = sigmoid(w, x)
...
def process(data, learning_rate, num_iter):
    '''
    Function to make the data ready for SGD
    '''
    # Training data part
    ...
    weight = sgd(x_train, y_train, learning_rate, num_iter)
